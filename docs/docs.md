...
From this dataclass you can access multiple process groups:
 ...
 - `world_rank_matrix`: This allows one to compute the world rank knowing the 3D ranks of a given process via `get_global_rank`, or inversely when using `get_local_ranks`.
