...
def create_config(
    ...
    # Parallelism settings (required args)
    ...
    optional_updates = {
        ...
        "optimizer": {"zero_stage": zero_stage},
        "parallelism": {"tp_mode": tp_mode}
    }

    for section, values in optional_updates.items():
        for key, value in values.items():
            if value is not None:
                if isinstance(value, dict):  # Handle nested model_config
                    for sub_key, sub_value in value.items():
                        if sub_key == "hidden_size" or sub_value == "hidden_size":
                            assert 1 == 1

                        if sub_value is not None:
                            config[section][key][sub_key] = sub_value
                else:
    ...
    #     config["model"]["model_config"]["intermediate_size"] < 10_000
    # )
    config["model"]["model_config"]["tie_word_embeddings"] = True

    # Build run name
    run_name_parts = [
        human_format(estimate_num_params(
        ...
        f"dp{dp}", f"tp{tp}", f"pp{pp}",
        f"ep{expert_parallel_size}", f"etp{expert_tensor_parallel_size}", f"edp{expert_data_parallel_size}",
        f"num_experts{num_experts}", f"topk{topk}",
        f"acc{batch_accum}" if batch_accum is not None else "",
        f"mbs{micro_batch_size}" if micro_batch_size is not None else "",
        f"seq{seq_len}" if seq_len is not None else "",
        f"zero{zero_stage}" if zero_stage is not None else "",
        f"tpmode{tp_mode[:3]}" if tp_mode else "",
        f"vocab{config['model']['model_config']['vocab_size']//1000}k"
    ]

    config["general"]["run"] = "_".join(filter(None, run_name_parts))
...
def main():
    ...
    num_experts_configs = [4, 16, 32, 64]
    # NOTE: qwen use has a topk of 4, qwen3 use has a topk of 8
    topk_configs = [1, 2, 4, 8]

    # Define configurations to test
    configurations = []

    for dp, tp, pp, ep, etp, edp in parallel_configs:
        for num_experts in num_experts_configs:
            for topk in topk_configs:
                configurations.append((dp, tp, pp, ep, etp, edp, num_experts, topk))
    ...
    generated_scripts = []
    configs = []
    for dp, tp, pp, ep, etp, edp, num_experts, topk in tqdm(configurations, desc="Generating configs and scripts"):
        try:
            # Create config
