...
class CoreAttention(nn.Module):
    def __init__(self, config: MixtralConfig, parallel_config: Optional[ParallelismArgs], layer_idx: int):
        super().__init__()
        assert (
...
class Embedding(nn.Module, AttachableStore):
    def __init__(self, tp_pg: dist.ProcessGroup, config: MixtralConfig, parallel_config: Optional[ParallelismArgs]):
        super().__init__()
...
class MixtralModel(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.ModuleList(
            [
                PipelineBlock(
                    p2p=self.p2p,
                    module_builder=MixtralDecoderLayer,
                    module_kwargs={
                        "config": config,
                        "parallel_config": parallel_config,
                        "tp_pg": parallel_context.tp_pg,
    ...
    def get_block_compute_costs(self):
        ...
        block_compute_costs = {
            ...
            MixtralDecoderLayer: 4 * model_config.num_attention_heads * d_qkv * model_config.hidden_size
...
class MixtralForTraining(NanotronModel):
    def __init__(
        self,
        config: MixtralConfig,
        parallel_context: ParallelContext,
        ...
        super().__init__()
        self.model = MixtralModel(config=config, parallel_context=parallel_context, parallel_config=parallel_config)
