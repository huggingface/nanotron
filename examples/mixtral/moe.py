...
from config_mixtral import MixtralConfig as Config
...
class ParallelDroplessMLP(torch.nn.Module):
    def __init__(
        ...
        if self.experts_per_rank == 1:
            ...
            self.mlp = SparseGLU(config=config, parallel_config=parallel_config, parallel_context=parallel_context)
...
class SparseGLU(SparseMLP):
    def __init__(
        self, config: Config, parallel_config: Optional[ParallelismArgs], parallel_context: "ParallelContext"
    ):
