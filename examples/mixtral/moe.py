...
class ParallelDroplessMLP(torch.nn.Module):
    def __init__(
        ...
        if self.experts_per_rank == 1:
            ...
            self.mlp = SparseGLU(config=config, parallel_config=parallel_config, tp_pg=tp_pg)
...
class SparseGLU(SparseMLP):
    def __init__(self, config: MixtralConfig, parallel_config: Optional[ParallelismArgs], tp_pg: dist.ProcessGroup):
