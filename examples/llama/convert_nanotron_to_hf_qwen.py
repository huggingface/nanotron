...
def get_hf_config(config: NanotronConfigs) -> HFLlamaConfig | HFQwen2Config:
    ...
    if isinstance(config, NanotronQwen2Config):
        # Handle sliding window for Qwen
...
def convert_checkpoint_and_save(
    ...
    with init_on_device_and_dtype(torch.device("cuda"), torch.bfloat16):
        ...
        if isinstance(model_config, NanotronQwen2Config):
            hf_model = Qwen2ForCausalLM._from_config(model_config_hf)
        else:
...
def check_converted_model_generation(save_path: Path):
    ...
    if "qwen" in str(save_path).lower():
        model = Qwen2ForCausalLM.from_pretrained(save_path).cuda().bfloat16()
    else:
