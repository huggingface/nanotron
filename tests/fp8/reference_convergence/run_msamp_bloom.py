...
def main():
    ...
    dataset = dataset.map(
        lambda x: tokenizer(x["inputs"], padding="max_length", truncation=True, max_length=SEQ_LEN, return_tensors="pt")
    )
    dataset.set_format(type="torch", columns=["input_ids", "attention_mask"])
    dataloaders = DataLoader(dataset["train"], batch_size=BATCH_SIZE, shuffle=False)

    config = BloomConfig(n_layer=N_LAYERS, hidden_size=HIDDEN_SIZE, n_head=N_HEADS, slow_but_exact=True, initializer_range=1)
