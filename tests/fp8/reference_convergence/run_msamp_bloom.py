...
def main():
    ...
    dataset = dataset.map(
        lambda x: tokenizer(x["inputs"], padding="max_length", truncation=True, max_length=128, return_tensors="pt")
    )
    dataset.set_format(type="torch", columns=["input_ids", "attention_mask"])
    dataloaders = DataLoader(dataset["train"], batch_size=16, shuffle=True)
    ...
    for _ in range(N_EPOCHS):
        for step, inputs in enumerate(dataloaders):
            print(f"step: {step} \n")
            # NOTE: move inputs to cuda
            inputs = {k: v.squeeze(dim=1).to("cuda") for k, v in inputs.items()}
