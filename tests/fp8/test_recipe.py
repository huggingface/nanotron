...
def test_fp8_training_recipe():
    recipe = FP8TrainingRecipe(
        linear=FP8LinearRecipe(
            ...
            weight=FP8TensorRecipe(dtype=DTypes.FP8E4M3, margin=0, interval=16, is_dynamic_scaling=False),
            ...
            weight_grad=FP8TensorRecipe(dtype=DTypes.FP8E4M3, margin=0, interval=16, is_dynamic_scaling=True),
            output_grad=FP8TensorRecipe(dtype=DTypes.FP8E5M2, margin=0, interval=1, is_dynamic_scaling=True),
    ...
    assert recipe.linear.input.is_dynamic_scaling is True
    ...
    assert recipe.linear.input_grad.is_dynamic_scaling is False
    ...
    assert recipe.linear.weight_grad.is_dynamic_scaling is True
    ...
    assert recipe.linear.output_grad.is_dynamic_scaling is True
