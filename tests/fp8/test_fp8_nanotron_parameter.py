...
def test_create_fp8_parameter(dtype):
    tensor = FP8Tensor(torch.randn(16, 16, device="cuda", dtype=torch.float32), dtype)
    parameter = NanotronParameter(tensor)

    assert parameter.requires_grad is False
    assert parameter.dtype == parameter.data.dtype
    assert parameter.data is tensor

    # assert isinstance(parameter, FP8Meta)
...
def test_setting_fp8_gradient_to_fp8_parameter(dtype, grad_dtype):
    ...
    parameter.grad = fp8_grad

    assert parameter.grad is fp8_grad
...
def test_set_data_in_fp8_parameter(dtype):
    ...
    new_data = FP8Tensor(torch.randn(16, 16, device="cuda", dtype=torch.float32), dtype=dtype)

    parameter.data = new_data

    assert parameter.data is new_data

    # TODO(xrsrke): expect changes in metadata
...
def _test_create_sharded_fp8_parameter(parallel_context: ParallelContext, dtype: DTypes):
    ...
    param = create_sharded_fp8_parameter(parameter, parallel_context)
