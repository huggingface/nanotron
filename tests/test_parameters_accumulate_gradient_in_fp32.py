...
def _test_ddp_with_grad_accum_in_fp32(
    ...
    for train_iter in range(train_iterations):
        ...
        for accum_step in range(accumulation_steps - 1):
            ...
            for name, param in model_ddp.named_parameters():
                ...
                with assert_fail_except_rank_with(AssertionError, rank_exception=0, pg=dp_group):
                    assert_tensor_synced_across_pg(grad, dp_group)
                with assert_fail_except_rank_with(AssertionError, rank_exception=0, pg=dp_group):
                    assert_tensor_synced_across_pg(fp32_grad_bucket, dp_group)
        ...
        for name, param in model_ddp.named_parameters():
            ...
            dist.all_reduce(model_ddp_accum_ref[name], group=dp_group, op=dist.ReduceOp.AVG)
        ...
        for name, param in model_ddp_fp32_accum.named_parameters():
            ...
            assert_tensor_synced_across_pg(grad, dp_group)
            assert_tensor_synced_across_pg(grad_fp32_accum, dp_group)
...
def _test_tied_weights_sync_with_grad_accum_in_fp32(
    ...
    p2p = P2P(pg=pp_group, device=device)
    ...
    for mdl in [model, reference_model]:
        ...
        with init_on_device_and_dtype(device, dtype):
            assert pp_world_size == len(mdl.mlp)
            for pp_rank, non_linear in zip(range(pp_world_size), mdl.mlp):
                non_linear.linear.build_and_set_rank(pp_rank=pp_rank)
                non_linear.activation.build_and_set_rank(pp_rank=pp_rank)
            mdl.loss.build_and_set_rank(pp_rank=pp_world_size - 1)
        ...
        tie_parameters(
            ...
            ties=[
                (
                    target,
                    (parallel_context.world_rank_matrix[get_pp_rank_of(target, module=mdl), dp_rank, tp_rank],),
                )
                for target in [f"mlp.{pp_rank}.linear.pp_block.weight" for pp_rank in range(pp_world_size)]
            ],
    ...
    # DDP
    dp_group = parallel_context.get_group(ParallelMode.DATA)
    model_ddp = torch.nn.parallel.DistributedDataParallel(model, process_group=dp_group)
    ...
    def forward_backward_reference(mdl, micro_batch):
        pipeline_engine.train_batch_iter(
            mdl, pg=pp_group, batch=[micro_batch], nb_microbatches=1, grad_accumulator=None
        )
    ...
    for name, param in reference_model.named_parameters():
        ...
        dist.all_reduce(reference_model_accum_ref[name], group=dp_group, op=dist.ReduceOp.AVG)

    ## Model iteration step
    pipeline_engine.train_batch_iter(
        model_ddp, pg=pp_group, batch=batch, nb_microbatches=n_micro_batches_per_batch, grad_accumulator=accumulator
    )
    for name, param in model_ddp.module.named_parameters():
        ...
        if not reduce_scatter:
            # Check that the gradients are synchronized across DP
            assert_tensor_synced_across_pg(fp32_grad, dp_group)
