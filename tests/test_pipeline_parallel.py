...
def _test_pipeline_engine(parallel_context: ParallelContext, pipeline_engine: PipelineEngine):
    ...
    n_micro_batches_per_batch = pp_world_size + 5
...
def _test_pipeline_engine_with_tensor_that_does_not_require_grad(
    ...
    # Set the ranks
    pp_world_size = parallel_context.get_world_size(ParallelMode.PIPELINE)
    assert len(model.mlp) == pp_world_size + 1
    # An additional mlp is in the end
    mlp_index_pp_rank = [(i, i) for i in range(pp_world_size)] + [(pp_world_size, pp_world_size - 1)]
...
def _test_pipeline_engine_diamond(parallel_context: ParallelContext, pipeline_engine: PipelineEngine):
    ...
    # Set the ranks
    pp_world_size = parallel_context.get_world_size(ParallelMode.PIPELINE)
    assert pp_world_size == len([model.dense_bottom, model.dense_left, model.dense_right, model.dense_top])
    assert pp_world_size == 4
