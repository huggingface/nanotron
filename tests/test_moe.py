...
from nanotron.nn.moe import AllToAllDispatcher, GroupedMLP, Qwen2MoELayer
...
def _test_all_to_all_dispatcher(rank, world_size, port, inputs, routing_indices):
    ...
    inputs = torch.chunk(inputs, world_size, dim=0)[ep_rank].cuda()
...
def test_all_to_all_dispatcher():
    ...
    mp.spawn(
        ...
        args=(world_size, port, inputs, routing_indices),
        nprocs=world_size,
    )
