    ...
    def __next__(self):
        ...
        if self.microbatch_idx == 0:
            ...
            for domain_index, (idxs, domain_batch_size) in enumerate(
                ...
                #     torch.tensor(self.domain_counters, device="cuda"), self.parallel_context.world_pg, msg=lambda err: f"domain_counters are not synced across global ranks {err}"
                # )

                # assert_tensor_synced_across_pg(
                #     torch.tensor(self.domain_counters, device="cuda"), self.parallel_context.dp_pg, msg=lambda err: f"domain_counters are not synced across dp ranks {err}"
                # )
        ...
        # assert_tensor_synced_across_pg(
        #     torch.tensor(self.domain_batch_sizes, device="cuda"), self.parallel_context.world_pg, msg=lambda err: f"domain_counters are not synced across global ranks {err}"
        # )

        # assert_tensor_synced_across_pg(
        #     torch.tensor(self.batch, device="cuda"), self.parallel_context.world_pg, msg=lambda err: f"domain_counters are not synced across global ranks {err}"
        # )
        ...
        if dp_end_idx > len(self.batch):
            raise StopIteration(f"dp_end_idx > len(batch), dp_end_idx: {dp_end_idx}, len(batch): {len(self.batch)} \n")
        ...
        if self.microbatch_idx == self.num_microbatches - 1:
            ...
            # print(f"rank={self.rank}, microbatch_idx={self.microbatch_idx}, reset microbatch_idx to 0 \n")
        else:
            self.microbatch_idx += 1
            # print(f"rank={self.rank}, microbatch_idx={self.microbatch_idx}, increase microbatch_idx by 1 \n")
