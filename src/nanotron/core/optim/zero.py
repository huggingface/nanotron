...
class ZeroDistributedOptimizer(InheritFromOtherOptimizer):
    ...
    def __init__(
        ...
        self.parallel_context = parallel_context  # DP process group
        ...
        current_dp_rank = parallel_context.get_local_rank(ParallelMode.DATA)
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        expected_allocated = sum(
            param.numel() * param.element_size() * 2 + param.numel() * param.element_size() * 2 / dp_world_size
    ...
    def _partition_parameters(self) -> Dict[str, Dict[int, Tuple[int, int]]]:
        ...
        for name, param in named_params:
            ...
            numel = param.numel()
            padded_numel_per_dp = (numel - 1) // dp_world_size + 1
            sizes = np.full(shape=(dp_world_size), fill_value=padded_numel_per_dp)
            remainder = padded_numel_per_dp * dp_world_size - numel
            ...
            assert len(end_offsets) == dp_world_size
    ...
    def _all_gather_params(self):
        ...
        current_dp_rank = self.parallel_context.get_local_rank(ParallelMode.DATA)
