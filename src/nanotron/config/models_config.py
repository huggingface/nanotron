...
class MoEConfig:
    ...
    router_aux_loss_coef: float = (
        0.01  # Scaling coefficient for the aux loss. A starting value of 1e-2 is recommended.
    )
    ...
    moe_z_loss_coeff: Optional[
        float
    ] = None  # Scaling coefficient for the z-loss. A starting value of 1e-3 is recommended.
    ...
    def __post_init__(self):
        ...
        if (
            ...
            and self.router_load_balancing_type != "sinkhorn"
        ):
...
class Qwen2Config:
    ...
    rope_seq_len_interpolation_factor: Optional[
        float
    ...
    def __post_init__(self):
        ...
        if self.max_position_embeddings > 4096:
            assert (
                self.rope_seq_len_interpolation_factor == self.max_position_embeddings / 4096
            ), f"rope_seq_len_interpolation_factor must be equal to max_position_embeddings / 4096 = {self.max_position_embeddings / 4096}"
