...
def init_optimizer_and_grad_accumulator(
    ...
    def basic_optimizer_builder(named_param_groups):
        ...
        elif optimizer_args.optimizer_factory.name == "custom_adamW":
            from nanotron.optim.adamw import CustomAdamW

            log_rank(
                "[DEBUGGING] Using CustomAdamW",
                logger=logger,
                level=logging.WARNING,
...
def track_weight_and_grad_stats(name: str, module: nn.Module, parallel_context: ParallelContext):
    def compute_stats(tensors, metrics: List[str] = ["amax", "mean", "std", "var", "norm"]):
        # from nanotron.fp8.utils import compute_stats
