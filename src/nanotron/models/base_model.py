...
class NanotronModel(nn.Module, metaclass=ABCMeta):
    ...
    def named_parameters(self, prefix: str = '', recurse: bool = True, remove_duplicate: bool = True) -> Iterator[Tuple[str, NanotronParameter]]:
    ...
    def get_named_params_with_tied(self) -> Iterable[Tuple[str, NanotronParameter]]:
        named_parameters = [
            (
                param.get_tied_info().get_full_name_from_module_id_to_prefix(module_id_to_prefix=self.module_id_to_prefix)
                if param.is_tied
                else name,
                param,
            )
