...
class Qwen2Attention(nn.Module):
    ...
    def _determine_seq_length_and_flat_ids(self, position_ids: torch.Tensor, inference_max_seqlen: Optional[int]):
        ...
        return seq_length, flat_position_ids

    def _forward_train_attn(
        self,
        qkv: torch.Tensor,
        position_ids: torch.Tensor,  # Original position_ids, likely [batch_size, seq_length]
        cu_seqlens: Optional[torch.Tensor],
    ):
        seq_length, flat_position_ids = self._determine_seq_length_and_flat_ids(position_ids, None) # inference_max_seqlen is None for training

        if self._use_qkv_packed:
            # _forward_packed uses self.training internally
            ...
            k = k.view(-1, self.local_num_kv_heads, self.head_dim)
            v = v.view(-1, self.local_num_kv_heads, self.head_dim)
            ...
            if self.config.no_rope_layer is None or (self.layer_idx + 1) % self.config.no_rope_layer != 0:
                ...
                q = self.rotary_emb.apply_rotary_pos_emb(q, rotary_pos_emb, seq_length=seq_length)
                k = self.rotary_emb.apply_rotary_pos_emb(k, rotary_pos_emb, seq_length=seq_length)
            else:
        ...
        return {"hidden_states": output, "position_ids": position_ids} # Return original position_ids

    def _forward_inference_attn(
        self,
        qkv: torch.Tensor,
        position_ids: torch.Tensor,  # Unpadded position_ids, likely [total_tokens]
        cu_seqlens: Optional[torch.Tensor],
        inference_max_seqlen: int,
    ):
        seq_length, flat_position_ids = self._determine_seq_length_and_flat_ids(position_ids, inference_max_seqlen)
        ...
        if self._use_qkv_packed:
            ...
            if self.config.no_rope_layer is None or (self.layer_idx + 1) % self.config.no_rope_layer != 0:
                # For inference, rotary_emb might be configured differently (e.g., for KV caching)
                # Currently, using flat_position_ids which are unpadded and continuous for each token.
                rotary_pos_emb = self.rotary_emb(
                    position_ids=flat_position_ids, seq_length=seq_length 
                )
                q = self.rotary_emb.apply_rotary_pos_emb(q, rotary_pos_emb, seq_length=seq_length)
                k = self.rotary_emb.apply_rotary_pos_emb(k, rotary_pos_emb, seq_length=seq_length)
            else:
            ...
            # cu_seqlens for inference are based on unpadded data
            attn_output = self.attention(
                q, k, v, position_ids=flat_position_ids, seq_length=seq_length, cu_seqlens=cu_seqlens
            )
            # TODO: KV Caching: return k, v here
        
        output = self.o_proj(attn_output)
        return {"hidden_states": output, "position_ids": position_ids} # Return unpadded position_ids for inference consistency

    def forward(
        self,
        hidden_states: torch.Tensor,  # [batch_size*seq_length, hidden_size] or [total_tokens, hidden_size]
        position_ids: torch.Tensor,  # [batch_size, seq_length] or [total_tokens]
        ...
        if self.training:
            return self._forward_train_attn(
                qkv,
                position_ids, # Original position_ids
                cu_seqlens,
            )
        else:
            assert inference_max_seqlen is not None, "inference_max_seqlen must be provided for inference"
            return self._forward_inference_attn(
                qkv,
                position_ids, # Unpadded position_ids
                cu_seqlens,
                inference_max_seqlen,
            )
...
class Qwen2Model(nn.Module):
    ...
    def _forward_inference(
        ...
        cu_seqlens = cu_seqlens.to(unpadded_input_ids.device if isinstance(unpadded_input_ids, torch.Tensor) else input_ids.device)
        ...
        # Pad logits back to original shape
    ...
    def forward(
        ...
        if self.training:
            return self._forward_train(input_ids=input_ids, position_ids=position_ids)
        else:
            return self._forward_inference(input_ids=input_ids, position_ids=position_ids)
