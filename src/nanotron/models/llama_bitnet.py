...
from nanotron.config import Config, LlamaBitNetConfig, ParallelismArgs
...
class MLPBitNet(nn.Module):
    def __init__(
        ...
        self.gate_up_proj = TensorParallelColumnLinearBitNet(
            config.hidden_size,
        ...
        self.down_proj = TensorParallelRowLinearBitNet(
...
class CoreAttention(nn.Module):
    def __init__(self, config: LlamaBitNetConfig, parallel_config: Optional[ParallelismArgs], layer_idx: int):
        super().__init__()
...
class CausalSelfAttentionBitNet(nn.Module, AttachableStore):
    def __init__(
        self,
        config: LlamaBitNetConfig,
        parallel_config: Optional[ParallelismArgs],
        ...
        self.qkv_proj = TensorParallelColumnLinearBitNet(
            self.d_model,
        ...
        self.o_proj = TensorParallelRowLinearBitNet(
...
class LlamaDecoderLayerBitNet(nn.Module):
    def __init__(
        self,
        config: LlamaBitNetConfig,
        parallel_config: Optional[ParallelismArgs],
...
class Embedding(nn.Module, AttachableStore):
    def __init__(self, tp_pg: dist.ProcessGroup, config: LlamaBitNetConfig, parallel_config: Optional[ParallelismArgs]):
        super().__init__()
...
class LlamaModelBitNet(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.ModuleList(
            [
                PipelineBlock(
                    p2p=self.p2p,
                    module_builder=LlamaDecoderLayerBitNet,
                    module_kwargs={
                        "config": config,
                        "parallel_config": parallel_config,
                        "tp_pg": parallel_context.tp_pg,
    ...
    def get_block_compute_costs(self):
        ...
        block_compute_costs = {
            # CausalSelfAttentionBitNet (qkv proj + attn out) + MLPBitNet
            LlamaDecoderLayerBitNet: 4 * model_config.num_attention_heads * d_qkv * model_config.hidden_size
            + 3 * d_ff * model_config.hidden_size,
            # This is the last lm_head
            TensorParallelColumnLinearBitNet: model_config.vocab_size * model_config.hidden_size,
        }
...
class LlamaForTrainingBitNet(NanotronModel):
    def __init__(
        self,
        config: LlamaBitNetConfig,
        parallel_context: ParallelContext,
        ...
        super().__init__()
        self.model = LlamaModelBitNet(config=config, parallel_context=parallel_context, parallel_config=parallel_config)
